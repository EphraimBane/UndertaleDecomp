...
if (con == 3 || con == 1 || con == 0.1 || con == 1.1)
{
    if instance_exists(OBJ_WRITER)
    {
        ...
        {
            wrx = (OBJ_WRITER.writingx - camera_0_x)
            dgx = (obj_dialoguer.x - camera_0_x)
            donkeykongismyfavoritemarvelsuperhero = 1
        }
        if (donkeykongismyfavoritemarvelsuperhero == 1)
        {
            OBJ_WRITER.myx = (camera_0_x + wrx)
            OBJ_WRITER.writingx = (camera_0_x + wrx)
            if instance_exists(obj_face)
            {
                obj_face.x = (camera_0_x + 24)
                OBJ_WRITER.myx = ((camera_0_x + wrx) + 58)
                OBJ_WRITER.writingx = ((camera_0_x + wrx) + 58)
            }
            obj_dialoguer.xx = camera_0_x
            obj_dialoguer.x = (camera_0_x + dgx)
        }
